...
// RB_testDoc.cpp: CRBtestDoc 클래스의 구현
//
...
IMPLEMENT_DYNCREATE(CRBtestDoc, CDocument)

BEGIN_MESSAGE_MAP(CRBtestDoc, CDocument)
END_MESSAGE_MAP()


// CRBtestDoc 생성/소멸

CRBtestDoc::CRBtestDoc() noexcept
{
	// TODO: 여기에 일회성 생성 코드를 추가합니다.

}

CRBtestDoc::~CRBtestDoc()
{
}

BOOL CRBtestDoc::OnNewDocument()
{
...
// CRBtestDoc serialization

void CRBtestDoc::Serialize(CArchive& ar)
{
	if (ar.IsStoring())
	{
...
// 축소판 그림을 지원합니다.
void CRBtestDoc::OnDrawThumbnail(CDC& dc, LPRECT lprcBounds)
{
...
// 검색 처리기를 지원합니다.
void CRBtestDoc::InitializeSearchContent()
{
...
void CRBtestDoc::SetSearchContent(const CString& value)
{
	if (value.IsEmpty())
	{
...
#ifdef _DEBUG
void CRBtestDoc::AssertValid() const
{
	CDocument::AssertValid();
}

void CRBtestDoc::Dump(CDumpContext& dc) const
{
	CDocument::Dump(dc);
}
#endif //_DEBUG
...
void CRBtestDoc::__erase_data_color(RB_NODE * n, RB_NODE * p, RB_NODE ** dpn)
{
	RB_NODE * s, *x, *y;
...
void  CRBtestDoc::__erase_data_change_node(RB_NODE ** dpn, RB_NODE * delNode, RB_NODE * replace)
{
...
void  CRBtestDoc::__erase_data(RB_NODE * delNode, RB_NODE ** dpn)
{
...
int  CRBtestDoc::erase_data(RB_NODE ** dpn, DATA rbdata)
{
	RB_NODE ** root = dpn;
...
void  CRBtestDoc::rotate_left(RB_NODE * pn, RB_NODE **dpn)
{
...
void  CRBtestDoc::rotate_right(RB_NODE * pn, RB_NODE **dpn)
{
...
void  CRBtestDoc::__insert_color(RB_NODE * npn, RB_NODE **dpn)
{
...
void  CRBtestDoc::__insert_link(RB_NODE * npn, RB_NODE * parent, RB_NODE **dpn)
{
	RB_SET_PARENT(npn, parent);
	npn->left = npn->right = 0;
	*dpn = npn;
}
int  CRBtestDoc::insert_data(RB_NODE ** dpn, RB_NODE * npn, DATA rbdata)
{
	RB_NODE ** root = dpn;
	RB_NODE * parent = NULL;
	RB_DATA * s;
...
void CRBtestDoc::MoveLine(HDC hdc, int sX, int sY, int eX, int eY) 
{ 
	MoveToEx(hdc, sX, sY, 0);
	LineTo(hdc, eX, eY); 
}
void CRBtestDoc::InitialInfo(DrawInfo * pDrawInfo)
{
...
void CRBtestDoc::EraseNode(DrawInfo * pDrawInfo)
{
	RECT rt;
...
void CRBtestDoc::__drawboard_predict_pos_node(HDC hdc, int line_sX, int line_sY, int line_eX, int line_eY,
...
void CRBtestDoc::__drawboard_draw_each_node(HDC hdc, TCHAR *  str, int cX, int cY, int size_node, int color)
{
...
void CRBtestDoc::__drawboard_new_node(DrawInfo * pDrawInfo, HDC hdc)
{
...
void CRBtestDoc::__drawboard_all_node(HDC hdc, RB_NODE * node, int size_node)
{
	TCHAR str[128] = { 0, };
	RB_DATA * d, *p;
	if (node == NULL)		return;
...
void CRBtestDoc::DrawBoard(DrawInfo * pDrawInfo, HDC hdc)
{
	TCHAR str[128] = { 0, };
...
void CRBtestDoc::__re_set_node_position(DrawInfo * pDrawInfo, RB_NODE * node, int * row, int * col)
{
	RB_DATA *d;
	if (node == NULL)		return;
...
void CRBtestDoc::ReSetNodePosition(DrawInfo * pDrawInfo)
{
	int row = -1, col = 0;

	__re_set_node_position(pDrawInfo, pDrawInfo->root, &row, &col);
}
void CRBtestDoc::Insert_Node(DrawInfo * pDrawInfo)
{
	RB_DATA * d;
...
void CRBtestDoc::Erase_Node(DrawInfo * pDrawInfo)
{
